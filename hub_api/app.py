...
def build() -> FastAPI:
    ...
    for module in (health, auth, oauth, me, models, tokens, uploads):
        app.include_router(module.router)
    return app


def _allow_origins(app: FastAPI, config: Settings) -> None:
    """Permit credentialed calls from the configured origins only."""
    app.add_middleware(
        CORSMiddleware,
